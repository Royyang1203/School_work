...
struct Node
{
    bool visited[MAX_N + 1] = {false}; //标记哪些点走了
    int path[MAX_N + 1];
    int s;       //第一个点
    int e;       //最后一个点
    int k = 0;   //走过的点数
...
public:
    ...
    Solution(vector<vector<double>> &data)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            city[i + 1].x = data[i][1];
            city[i + 1].y = data[i][2];
            // cout << city[i + 1].x << " " << city[i + 1].y << endl;
        }
...
int main()
{
    ...
    printf("Min_cost: %.3lf", sol->result.sumv);
