...
def get_single_stock_data_daily(root_path, symbol):
    ...
    '''
    df, lastUpdateTime = queryStock(
        root_path, "DB_STOCK", "SHEET_CHN", "_DAILY", symbol, "daily_update")
    df.index = pd.to_datetime(df.index)

    suspended_day = pd.Timestamp(
...
def get_single_stock_data_weekly(root_path, symbol):
    ...
    '''
    df, lastUpdateTime = queryStock(
        root_path, "DB_STOCK", "SHEET_CHN", "_WEEKLY", symbol, "weekly_update")
    df.index = pd.to_datetime(df.index)
...
def get_single_stock_data_monthly(root_path, symbol):
    ...
    '''
    df, lastUpdateTime = queryStock(
        root_path, "DB_STOCK", "SHEET_CHN", "_MONTHLY", symbol, "monthly_update")
    df.index = pd.to_datetime(df.index)
...
def KDJ(df):
    ...
    low_list.fillna(value=df['low'].expanding(
        min_periods=1).min(), inplace=True)
    high_list = df['high'].rolling(center=False, window=9).max()
    high_list.fillna(value=df['high'].expanding(
        min_periods=1).max(), inplace=True)
    rsv = (df['close'] - low_list) / (high_list - low_list) * 100
    df['kdj_k'] = rsv.ewm(min_periods=0, adjust=True,
                          ignore_na=False, com=2).mean()
    df['kdj_d'] = df['kdj_k'].ewm(
        min_periods=0, adjust=True, ignore_na=False, com=2).mean()
    ...
    try:
        if not set(ma_names).issubset(df.columns):
            for idx, item in enumerate(ma_names):
                df[item] = df['close'].rolling(
                    window=ma_parameters[idx], center=False).mean()
...
# (df['macd_dif'][-input_3:].min() < input_2) & \
    ...
    return (df['macd_dif'][index] > input_1) & \
           ...
           ((df['macd_dea'][index-1] > df['macd_dif'][index-1]) |
            (abs(df['macd_dea'][index-1] - df['macd_dif'][index-1]) < 0.007))
...
def inner_processing_stock_data(symbol, input_data, window, day_selection, week_selection, month_selection):
    # input_data[input_data['volume'] > 0].copy()
    day_data = input_data['daily']
...
def processing_stock_data(root_path, symbol, window, day_selection, week_selection, month_selection):
    ...
    if data_daily.empty:
        return startTime
    if len(data_daily) < 60 + window:
        return startTime

    data = {"daily": data_daily, "weekly": data_weekly, "monthly": data_monthly}

    inner_processing_stock_data(
        symbol, data, window, day_selection, week_selection, month_selection)
...
def process_all_stocks_data(root_path, window=1):
    ...
    for symbol in symbols:
        startTime = processing_stock_data(
            root_path, symbol, window, day_selection, week_selection, month_selection)
        outMessage = '%-*s processed in:  %.4s seconds' % (
            6, symbol, (time.time() - startTime))
...
def calBasic():
    ...
    baseData = baseData[(baseData.pe < pe) & (
        baseData.gpr > gpr) & (baseData.npr > npr)]
    ...
    roe_list = list(set(main_symbols[0]) & set(
        main_symbols[1]) & set(main_symbols[2]))
    roe_list = [str(item).zfill(6) for item in roe_list]

    nav_list = list(set(grow_symbols[0]) & set(
        grow_symbols[1]) & set(grow_symbols[2]))
